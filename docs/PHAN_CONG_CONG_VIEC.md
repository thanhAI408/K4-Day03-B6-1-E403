...
| :-------------------------------------------- | :------------------------- | :--------------------------------------------------------------------------------------------------------- | :------------------- |
| **Role 1: Product Architect**           | `config/test_cases.json` | Định hướng bài toán & soạn bộ câu test case                                                       | `Pham Quoc Bao`      |
| **Role 2: Tool Engineer**               | `src/tools.py`           | Định nghĩa các công cụ (Tools) cho Agent                                                             | `Tran Hoang Long` |
| **Role 3: Prompt Engineer**             | `src/prompts.py`         | Viết ReAct System Prompt & phanh Guardrails                                                               | `Nguyen Van Thanh` |
| **Role 4: Core Developer / Integrator** | `src/app.py`             | **Đầu mối kéo code/file của nhóm (`git pull`), Vibe Code lắp ráp thành App hoàn chỉnh** | `Nguyen Van Thanh` |
| **Role 5: Observability**               | `docs/trace_eval.md`     | Lập bảng Scoring Matrix & Soi nhật ký Trace Log                                                        | `Pham Quoc Bao`      |
...
- [x] **Role 1 & Cả nhóm**: **Tự do lựa chọn 1 chủ đề bài toán thực tế** mà nhóm hào hứng nhất (Chủ đề được chọn từ [DANH_SACH_DE_TAI.md](file:///c:/Users/Lil'Pao0/Documents/Vin_AI/K4-Day03-B6-1-E403/docs/DANH_SACH_DE_TAI.md): **Đề tài 5 - Trợ Lý Tra Cứu Đơn Hàng & Xử Lý Đổi Trả**).
- [x] **Role 5**: Điền bảng **Scoring Matrix** (chấm 1–5 điểm cho 4 tiêu chí) vào `docs/trace_eval.md`.
- [x] **Role 2**: Liệt kê tên các công cụ sẽ tạo trong `src/tools.py` phù hợp với chủ đề nhóm đã chọn (`get_order_status`, `check_return_policy`, `process_return`).
- [x] **Role 3**: Xác định các trường hợp tool có thể bị lỗi (Failure Modes - mã đơn hàng không tồn tại, đơn hàng quá hạn đổi trả).
...
- [x] **Role 2**: Dùng AI bổ sung Docstring / Mô tả chuẩn cho các hàm trong `src/tools.py` (12 tools với đầy đủ docstring Args/Returns).
...
<<<<<<< HEAD
- [x] ⚔️ **Đội Tấn Công (Đại diện/Học viên được gọi)**: Mang các câu test case của nhóm mình sang "xả" vào Agent của Nhóm bạn để kiểm thử khả năng chịu lỗi. (Đã có 4 câu bẫy hiểm hóc trong `docs/MOC_4_PHONG_THU_TAN_CONG.md`).
- [x] 🛡️ **Đội Phòng Thủ**: Quan sát Agent nhóm mình phản ứng trước câu hỏi của nhóm bạn. Kiểm tra xem Guardrail bảo vệ an toàn không. (Đã chuẩn bị 4 kịch bản phản biện bảo vệ Agent trong `docs/MOC_4_PHONG_THU_TAN_CONG.md`).
=======
...
>>>>>>> 075ad822700283a26fbe77da9793e65885c7ca81
